...
class UserPopulator:
    ...
    def _populate_from_oidc(self, attributes: Dict[str, Any]) -> None:
        ...
        self.owner.affiliation = getattr(settings, "OIDC_DEFAULT_AFFILIATION", DEFAULT_AFFILIATION)
    ...
    def _apply_ldap_entry(self, entry: Any) -> None:
        ...
        self.owner.affiliation = self._get_ldap_value(entry, "primaryAffiliation", DEFAULT_AFFILIATION)
    ...
    def _assign_access_groups(self, groups: List[str]) -> None:
        ...
        for group_code in groups:
            ...
            if create_group_from_groups:
                accessgroup, created = AccessGroup.objects.get_or_create(code_name=group_code)
                if created:
...
def get_ldap_conn() -> Optional[Connection]:
    """Open and get LDAP info."""
