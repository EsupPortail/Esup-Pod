...
| **`SECRET_KEY`**             |    ✅    | _(None)_             | Django security key. **Must be secret in production.**            |
...
| **`MYSQL_ROOT_PASSWORD`** |    ❌    | _(None)_ | Root password for DB container initialization.    |
...
| **`AUTH_LDAP_BIND_PASSWORD`** | _(None)_              | Password for the Bind DN user. |
...
3.  **Update Documentation**: Use the [Management Commands](core/MANAGEMENT_COMMANDS.md) to sync the new setting with the JSON repository and generate updated documentation.
