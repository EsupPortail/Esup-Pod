...
_Validation:_ The `clean` and `save` methods ensure `time_end` is strictly greater than `time_start`. If `LINK_SUPERPOSITION` is enabled in the configuration, URLs within `content` are automatically converted to HTML links.
