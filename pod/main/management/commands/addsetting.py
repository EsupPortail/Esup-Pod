...
class Command(BaseCommand):
    ...
    def get_setting(self, options, config_part):
        ...
        if settings.get(options['setting_name']):
            ...
            self.stdout.write(
                self.style.WARNING(
                    'Setting found in json file, you will modify it !'
                )
            )
    ...
    def save_setting(self, options, config_part, setting):
        ...
        if options['app_name'] == 'pod':
            data[0]["configuration_pod"]["description"][
                config_part
            ]["settings"][options['setting_name']] = setting
        else:
            data[0]["configuration_apps"]["description"][
                options['app_name']
            ]["settings"][options['setting_name']] = setting
        os.remove(filename)
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS('App name "%s"' % options['app_name'])
        )
