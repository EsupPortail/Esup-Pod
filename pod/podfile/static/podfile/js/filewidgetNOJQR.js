...
if (typeof loaded == "undefined") {
  ...
  document.addEventListener("show.bs.modal", (event) => {
    ...
    modal.querySelectorAll("form").style.display = "none";
  ...
  async function reloadRemoveBtn() {
  ...
  async function reloadAddBtn(searchTerm) {
  ...
  document.addEventListener("show.bs.modal", (event) => {
    ...
    let modal = button.dataset.target;
  ...
  function show_folder_files(data) {
    ...
    if (data.list_element) {
      document.getElementsByClassName("files").innerHTML = data.list_element;
      ...
      document
        .querySelector("#folder_" + data.folder_id + " img")
        .setAttribute("src", static_url + "podfile/images/opened_folder.png");
  ...
  document.addEventListener("show.bs.modal", (event) => {
    if (!e.target(".podfilemodal")) return;
    event.stopPropagation();
    setTimeout(function () {
      initFolders();
    }, 500);
