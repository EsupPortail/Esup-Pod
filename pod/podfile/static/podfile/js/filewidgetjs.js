...
if (typeof loaded == "undefined") {
  ...
  $(document).on("click", "a.folder", function (e) {
    e.preventDefault();
    $("#podfile #list_folders_sub a.folder-opened").removeClass(
      "folder-opened"
    );
  ...
  $(document).on(
    "submit",
    "form#formchangeimage, form#formchangefile, form#formuploadfile",
    function (e) {
      e.preventDefault();
      //alert('FORM');
      $(this).hide();
  ...
  $(document).on("show.bs.modal", "#folderModalCenter", function (event) {
    ...
    switch (filetype) {
      ...
      default: // Extract info from data-* attributes
        ...
        let oldname = button.data("oldname");
        // If necessary, you could initiate an AJAX request here (and then do the updating in a callback).
        // Update the modal's content. We'll use jQuery here, but you could use a data binding library or other methods instead.
        modal
          .find(".modal-body input#folderInputName")
          .val(oldname)
          .trigger("focus");
  ...
  function show_folder_files(data) {
    ...
    if (data.list_element) {
      ...
      $("img.directory-image").attr(
        "src",
        static_url + "podfile/images/folder.png"
      );
      $("img.home-image").attr(
        "src",
        static_url + "podfile/images/home_folder.png"
      );
      $("#folder_" + data.folder_id).addClass("font-weight-bold");
      $("#folder_" + data.folder_id + " img").attr(
        "src",
  ...
  $(document).on(
    "click",
    "#list_folders_sub .view-more-container a#more",
    function (e) {
