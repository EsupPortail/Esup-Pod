...
async function fetchSingleFilter(filterName, searchTerm = "") {
  let url = AVAILABLE_FILTER_URL.replace(
  ...
  try {
    ...
    if (!data || typeof data !== "object" || !Array.isArray(data[key])) {
      ...
      throw new Error(
        `Malformed API response: missing or invalid array for key '${key}'`,
      );
    }
    ...
    console.error(
      `Error fetching filter '${filterName}' with term '${searchTerm}' from ${url}:`,
      error,
    );
    return [];
  }
