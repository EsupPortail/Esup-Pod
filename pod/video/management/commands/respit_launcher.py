...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        if USE_RESPIT:
            ...
            notif_list = []
            for p in Video.objects.raw('SELECT * FROM video_video WHERE SUBSTRING (title, 1, 7) != "Archivé"'):
                ...
                if ((p.date_delete - timedelta(days=higher_warn+1)) <= (date.today())):
                    ...
                    if (self.dry_mode == False):
                        ...
                        p.save()
                        self.stdout.write(self.style.SUCCESS("Add "+str(daysmore)+" days to the delete_date"))
                        ...
                        self.stdout.write(self.style.SUCCESS("DRY MODE : Simultate a Adding of "+str(daysmore)+" days to the delete_date"))
                        self.stdout.write(self.style.SUCCESS(str(p.date_delete + timedelta(daysmore))))
                        self.stdout.write("")
                else:
                    self.stdout.write("Video '"+p.title+"' a une date delete le "+str(p.date_delete)+". C'est dans plus de "+str(int(higher_warn+1))+" jours. Rien à faire.")


            if (self.dry_mode == False):
                if not notif_list:
                    self.stdout.write("\n")
                    self.stdout.write("** Au répis calculé. Pas d'envoie de mail aux managers **")
                else:
                    self.stdout.write("\n")
                    self.stdout.write("** Envoie de mail aux managers. **")
                    msg_html = "Hello ! La deadline des vidéos suviantes a été reporté le selon les consignes du modèle "+RESPIT_MODEL+" : \n"
