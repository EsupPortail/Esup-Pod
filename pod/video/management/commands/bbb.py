...
BBB_USERNAME_FORMAT = getattr(
    settings, "BBB_USERNAME_FORMAT", "first_name last_name"
)

# Number of days before removal the meetings
BBB_NUMBER_DAYS_BEFORE_DELETE = getattr(
    settings, "BBB_NUMBER_DAYS_BEFORE_DELETE", 0
)
...
def get_bbb_meetings_by_xml(html_message_error, message_error):
    ...
    try:
        ...
        returncode = xmldoc.getElementsByTagName("returncode")[
            0
        ].firstChild.data
...
def get_meeting(meeting, html_message_error, message_error):
    try:
        ...
        meetingName = meeting.getElementsByTagName("meetingName")[
            0
        ].firstChild.data
        ...
        for attendee in meeting.getElementsByTagName("attendee"):
            get_attendee(
                attendee, idActualMeeting, html_message_error, message_error
            )
...
def matching_bbb_pod_user(html_message_error, message_error):
    ...
    try:
        ...
        attendees = Attendee.objects.filter(user_id__isnull=True).order_by(
            "-id"
        )[:500]
...
def get_recording(
    recording, internal_meeting_id, html_message_error, message_error
):
    try:
...
def delete_old_meetings(html_message_error, message_error):
    ...
    try:
        ...
        if BBB_NUMBER_DAYS_BEFORE_DELETE == 0:
            print_if_debug(
                " - BBB_NUMBER_DAYS_BEFORE_DELETE = 0 " ": nothing to remove."
            )
        else:
