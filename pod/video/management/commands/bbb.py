...
BBB_NUMBER_DAYS_BEFORE_DELETE = getattr(
    settings, 'BBB_NUMBER_DAYS_BEFORE_DELETE',
    0
)
...
def delete_old_meetings(html_message_error, message_error):
    ...
    try:
        ...
        if BBB_NUMBER_DAYS_BEFORE_DELETE == 0:
            print_if_debug(" - BBB_NUMBER_DAYS_BEFORE_DELETE = 0 "
                           ": nothing to remove.")
        else:
            ...
            meetings = Meeting.objects.filter(encoding_step__lt=3,
                                              session_date__lte=date_removal
                                              ).order_by('id')
            for meeting in meetings:
                print_if_debug(" - Removal of this meeting. "
                               "internal_meeting_id : "
                               + meeting.internal_meeting_id + ".")
                meeting.delete()
    except Exception as e:
        err = "Problem to delete old meetings : " + str(e) + ". "\
            "" + traceback.format_exc()
        message_error += err + "\n"
        html_message_error += "<li>" + err + "</li>"
        print_if_debug(err)
