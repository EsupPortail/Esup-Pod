...
class Command(BaseCommand):
    ...
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        self.stdout.write(
            self.style.SUCCESS("***Start import Pod3 JSON file to Pod v4 database***")
        )
        ...
        if options["dry"]:
            if options["createDB"]:
                self.stdout.write(
                    self.style.SUCCESS(" - The command make createDB will be executed.")
                )
        else:
            if options["createDB"]:
    ...
    def execute_bash_commands(self) -> None:
        ...
        for command in commands:
            try:
                ...
                self.stdout.write(
                    self.style.SUCCESS(f"Command executed successfully: {command}")
                )
    ...
    def verify_json_file(self, json_file: str, options: Dict[str, Any]) -> bool:
        ...
        if options["dry"]:
            self.stdout.write(
                self.style.SUCCESS(f" - The JSON file {json_file} will be processed.")
            )

        if not os.path.exists(json_file):
            self.stdout.write(
                self.style.ERROR(f" - The JSON file {json_file} does not exist.")
            )
            return False
    ...
    def display_dry_run_tables(
        ...
        if options["onlytags"]:
            self.stdout.write(
                self.style.SUCCESS(
                    " - Only the tables useful for tags management will be processed."
                )
            )
        else:
            for table in data:
                ...
                if table in [
                    ...
                    if not options["onlytags"]:
                        try:
                            ...
                            self.stdout.write(
                                self.style.SUCCESS(
                                    f" - The table {table} has been processed."
                                )
                            )
                            ...
                            self.stdout.write(
                                self.style.ERROR(
                                    f" - The table {table} has not been processed. Error: {e}"
                                )
                            )

        if error:
            self.stdout.write(
                self.style.ERROR(f"Not all data has been imported from {json_file}.")
            )
        else:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Data has been successfully imported from {json_file}."
                )
            )
    ...
    def handle_exception(self, e: Exception, entity_id: int, entity_type: str) -> None:
        ...
        if "Too many connections" in str(e):
            self.stdout.write(
                self.style.ERROR(f"Error: tags error for {entity_type} {entity_id}: {e}.")
            )
        else:
            self.stdout.write(
                self.style.WARNING(
