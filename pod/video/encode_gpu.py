...
def get_cmd_gpu(format, codec, height, file):
    ...
    if format == "m3u8":
        ffmpeg_cmd = (
            ffmpeg_cmd
            + SUBTIME
            + end_360_m3u8.format(output_dir=VIDEOS_OUTPUT_DIR, output=filename)
        )
    else:
        ffmpeg_cmd = (
            ffmpeg_cmd
            + SUBTIME
            + end_360_mp4.format(output_dir=VIDEOS_OUTPUT_DIR, output=filename)
        )
    if height >= 720:
...
def get_cmd_cpu(format, codec, height, file):
    ffmpeg_cmd = ""
    ffmpeg_cmd = CPU.format(codec=codec, input=os.path.join(VIDEOS_DIR, file))
    ffmpeg_cmd = ffmpeg_cmd + COMMON
...
def encode(type, format, codec, height, file):
    ...
    if return_value is True:
        add_info_video(
            add_info_video_title, add_info_video_content, add_info_video_append
        )
...
if __name__ == "__main__":
    ...
    VIDEOS_OUTPUT_DIR = (
        os.path.join(VIDEOS_DIR, os.path.basename(VIDEOS_DIR).split("-")[-1])
        if args.dir
    ...
    if os.access(path_file, os.F_OK) and os.stat(path_file).st_size > 0:
        ...
        filename = "".join(
            (
                c
                for c in unicodedata.normalize("NFD", input_file)
                if unicodedata.category(c) != "Mn"
            )
        )
        filename = filename.replace(" ", "_")
        os.rename(
            "{}/{}".format(VIDEOS_DIR, input_file),
