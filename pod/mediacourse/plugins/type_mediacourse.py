# type_mediacourse.py : copie revisitée de type_audiovideocast.py
...
def save_video(recording, video_data, video_src):
    video = Video()
    video.owner = recording.recorder.user
    video.type = Type.objects.get(id=recording.recorder.type.id)
...
def open_zipfile(recording):
    try:
        zip = zipfile.ZipFile(recording.mediapath)
        return zip
...
def encode_recording(recording):
    ...
    media_name, ext = os.path.splitext(
        os.path.basename(recording.mediapath))
    ...
    os.rename(recording.mediapath, recording.mediapath+"_treated")
