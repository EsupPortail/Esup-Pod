...
VIDEO_TEST = getattr(
    settings, 'VIDEO_TEST',
    'pod/main/static/video_test/pod.mp4')

AUDIOVIDEOCAST_TEST = getattr(
    settings, 'AUDIOVIDEOCAST_TEST',
    'pod/main/static/video_test/pod.zip')
...
class PluginVideoTestCase(TestCase):
    ...
    def setUp(self):
        ...
        recorder1 = Recorder.objects.create(user=user, name="recorder1",
    ...
    def test_type_video_published_attributs(self):
        ...
        self.assertEquals(video.theme.all().count(),
                          recorder.theme.all().count())
        self.assertEquals(video.discipline.all().count(),
