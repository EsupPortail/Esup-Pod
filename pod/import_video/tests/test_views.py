...
class ExternalRecordingUploadTestView(TestCase):
    ...
    @patch("pod.import_video.views.YouTube")
    def test_recording_upload_post_request_youtube_http_403(self, mock_youtube):
        """Test Youtube upload error message with HTTP 403."""
        mock_youtube.side_effect = HTTPError(
        ...
        recording_yt = ExternalRecording.objects.get(name="test youtube recording1")
        self.user = User.objects.get(username="pod2")
        self.client.force_login(self.user)
        url = reverse(
            "import_video:upload_external_recording_to_pod",
            kwargs={"record_id": recording_yt.id},
        )
        response = self.client.post(
            url,
            {
        ...
        self.assertFalse(b"HTTP Error 403: Forbidden" in response.content)

    @patch("pod.import_video.views.save_video")
    @patch("pod.import_video.views.save_external_recording")
    @patch("pod.import_video.views.check_video_size")
    @patch("pod.import_video.views.YouTube")
    def test_recording_upload_post_request_youtube_client_fallback(
        self,
        mock_youtube,
        mock_check_video_size,
        mock_save_external_recording,
        mock_save_video,
    ):
        """Test Youtube upload retries with fallback client."""
        stream = Mock()
        stream.resolution = "720p"
        stream.filesize = 10
        stream.default_filename = "fallback.mp4"
        stream.download.return_value = None

        youtube_web = Mock()
        youtube_web.publish_date = "2026-01-01 00:00:00"
        youtube_web.title = "fallback title"
        youtube_web.streams = Mock()
        youtube_web.streams.filter.return_value.order_by.return_value.desc.return_value = [
            stream
        ]
        ...
        def youtube_side_effect(source_url, client):
            ...
            if client == "WEB":
                return youtube_web
            raise AssertionError("Unexpected YouTube client: %s" % client)
