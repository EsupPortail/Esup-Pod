...
var loadEnrichmentVTTfile = function(url, callback) {
    var getXhr = function() {
        ...
        catch(e){try { return new ActiveXObject("Msxml3.XMLHTTP");}
                catch(e){try { return new ActiveXObject("Msxml2.XMLHTTP.6.0"); }
      ...
      , setCue = function(index,data) {//console.log('setCue '+index)
        ...
        xhr.onreadystatechange = function() {
            if (this.readyState == 4) {
                var lines = xhr.responseText.split('\n')
                  ...
                  , c = 0;
                //// Check WebVTT
                if(lines[0].substr(0,6) != 'WEBVTT') { callback(cues); return false; }
...
var VideoSlides = function(items) {
	...
	this.slideBar=function() {
            ...
            for (let i = 0; i <= keys.length - 1; i++) {
                ...
                newslide.setAttribute('style','left: ' + slidebar_left + '%; width: ' + slidebar_width + '%; background-color: ' + slide_color[type]);
    ...
    if(document.getElementsByClassName('vjs-slide-manager').length > 0) {document.getElementsByClassName('vjs-slide-manager')[0].parentNode.removeChild(document.getElementsByClassName('vjs-slide-manager')[0]);}
...
const onPlayerReady = function(player, options) {
	try {player.addClass('vjs-slides');}
	catch(e) { if(player.className.indexOf('vjs-slides') < 0) player.className += ' vjs-slides'; }
