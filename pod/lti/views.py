...
class LTIAssignmentAddVideoView(LTIAuthMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(LTIAssignmentAddVideoView, self).get_context_data(**kwargs)
...
class LTIAssignmentGetVideoView(LTIAuthMixin, LoginRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        context = super(LTIAssignmentGetVideoView, self).get_context_data(**kwargs)
        if self.request.session.get("custom_video"):
            try:
                video = Video.objects.get(id=self.request.session.get("custom_video"))
