document.addEventListener("DOMContentLoaded", function () {
    ...
    let restrictedLabel = document.getElementsByClassName("field_is_restricted")[0];

    let change_restriction = (restrict) => {
        if (restrict === true) {
            restrictedCheckBox.checked = true;
            restrictedCheckBox.setAttribute("onclick", "return false");
            restrictedHelp.innerHTML = gettext("Restricted because the broadcaster is restricted");
            restrictedLabel.style.opacity = "0.5";
        } else {
            restrictedCheckBox.removeAttribute("onclick");
            restrictedHelp.innerHTML = gettext("If this box is checked, the event will only be accessible to authenticated users.");
    ...
    let getBroadcasterRestriction = () => {
        ...
        if (typeof broadcaster_id === "undefined" || broadcaster_id === "" || isNaN(broadcaster_id))
            return;
    ...
    document.getElementById("event_building").addEventListener('change', function () {
        ...
        fetch(
            ...
            .then((broadcasters) => {
                ...
                if (Object.entries(broadcasters).length === 0) {
                    ...
                    broadcastField.innerHTML += "<option value> " + gettext("No broadcaster set for this building") + "</option>";
                } else {
                    broadcastField.disabled = false;
                    for (const [key, value] of Object.entries(broadcasters)) {
                        broadcastField.innerHTML += '<option value="' + value.id + '">' + value.name + "</option>";
                    }
