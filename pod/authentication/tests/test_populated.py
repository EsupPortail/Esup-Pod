...
SHIB_URL = getattr(
    settings,
    "SHIB_URL", "https://univ.fr/Shibboleth.sso/WAYF")
...
class PopulatedShibTestCase(TestCase):
    ...
    def _authenticate_shib_user(self, u):
        ...
        # Authenticate user with valid shib_meta
        user = auth.authenticate(remote_user=u['username'], shib_meta=shib_meta)
    ...
    def test_make_profile(self):
        ...
        """ Test if user can be staff if SHIBBOLETH_STAFF_ALLOWED_DOMAINS is restrict """
        ...
        """ Test if user become staff if SHIBBOLETH_STAFF_ALLOWED_DOMAINS is restrict and contains his domain """
        ...
        for a in unstaffable_affiliation:
            self.assertFalse(a in settings.AFFILIATION_STAFF)
        user, shib_meta = self._authenticate_shib_user({
            'username': 'jdo@univ.fr',
