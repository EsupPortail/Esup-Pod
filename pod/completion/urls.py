...
urlpatterns = [
    url(
        r"^video_completion/(?P<slug>[\-\d\w]+)/$",
        video_completion,
        name="video_completion",
    ),
    url(
        r"^video_caption_maker/(?P<slug>[\-\d\w]+)/$",
        video_caption_maker,
        name="video_caption_maker",
    ),
    url(
        r"^video_completion_contributor/(?P<slug>[\-\d\w]+)/$",
        video_completion_contributor,
        name="video_completion_contributor",
    ),
    url(
        r"^video_completion_document/(?P<slug>[\-\d\w]+)/$",
        video_completion_document,
        name="video_completion_document",
    ),
    url(
        r"^video_completion_track/(?P<slug>[\-\d\w]+)/$",
        video_completion_track,
        name="video_completion_track",
    ),
    url(
        r"^video_completion_overlay/(?P<slug>[\-\d\w]+)/$",
