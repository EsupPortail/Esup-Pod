let PlaylistPlayer = {
  ...
  onPlayerEnd:function() {
    ...
    if (this.current_position != this.length || this.loop_on) {
      ...
      $(this.current_element).parent().children('.vdata').text(this.current_position)
      if (this.current_position != this.length) {
        this.current_position++;
        this.current_element = $('.playlist-videos div.row > div > div.card')[this.current_position - 1];
      } else if(this.loop_on) {
        this.current_position = 1;
        this.current_element = $('.playlist-videos div.row > div > div.card')[0];
      }
      ...
      $.ajax({
        ...
        if(json.status == 'ok') {
          ...
          if($('#video-form-wrapper').length == 0) {
            _this.formctn.append('<div id="video-form-wrapper" class="jumbotron"></div>')
          }
          ...
          $('#video-form-wrapper button[type="submit"]').click(function(e) {
            ...
            if(password == '') {
              $('#video-form-wrapper .invalid-feedback').html(this.strings.invalid_feedback_value.replace("'","\'")).show()
              return;
            }
            $.ajax({
              type: 'POST',
              ...
              if(json.status == "ok") {
                ...
                $('#video-form-wrapper .invalid-feedback').html(this.strings.invalid_feedback_password).show()
              }
            });
          })
  ...
  init:function(o) {
    ...
    this.chevron_up = $(this.current_element).parent().children('.vdata').html()
    this.auto_on = this.loop_on = false;

    const parameter = [/(playlist)\=([^&]+)/, /(auto)\=([^&]+)/, /(loop)\=([^&]+)/]
    ...
    player.on('ended', function() {_this.onPlayerEnd()})
  },

  headFiles: {
    plugins:{enrichment:null,chapter:null,vr360:null,overlay:null,overview:null},
    set: function(o) {
      for(let p in o) {
    ...
    getOrLoad: function(p,cb) {
      if(this.plugins[p].js) {
        if( $('#'+p+'_style_id').length == 0 ) {
          $('head').append('<link id="'+p+'_style_id" href="'+this.plugins[p].css+'" rel="stylesheet" />')
        }
    ...
    unload: function(p) { $('#'+p+'_style_id, #'+p+'_script_id').remove() },
    unloadCSS: function(p) { $('#'+p+'_style_id').remove() }
  },
  ...
  setPlayer : function(json) {
    ...
    $('form#video_count_form, ul.video-slides, ul#overlays, div.chapters-list').remove()
    player.dispose()
    ...
    player.ready(function() {
      ...
      // Enrichments
      if(has_enrichment) {
        $(player.el().getElementsByTagName('VIDEO')[0]).append('<track kind="metadata" src="'+json.enrichtracksrc+'" label="enrichment" />')
        _this.headFiles.getOrLoad('enrichment', function() {
          var tracks = player.el().getElementsByTagName('TRACK');
          for (var i = 0; i < tracks.length; i++) {
              if (tracks[i].kind === 'metadata' && tracks[i].label === 'enrichment') {
          ...
          player.on('loadedmetadata', function() {
            ...
            if(!metadataTrack.cues) { //Safari do not get cues
              ...
              if (typeof player.slides === "function") {
                try {player.slides(slide);} catch(e) {;}
              }
            }
          });
        })
      ...
      if(json.overview) {
        if(typeof player.vttThumbnails === 'function') {
          ...
          _this.headFiles.getOrLoad('overview', function() {
            try {
              player.vttThumbnails({src:_this.baseurl+json.overview});
            } catch(e) { console.error('Error in retrieving overview image : '+e) }
          })
        }
      }
      ...
      if(_this.is_iframe) {
        ...
        if(typeof resizeVideoJs === 'function') { resizeVideoJs() }
      }
      ...
      else if(json.src.m4a)
        player.src(json.src.m4a)
      ...
      $(_this.current_element).parent().children('.vdata').html(_this.chevron_up)
      let current_url = _this.current_element.children[1].children[0].href
      history.pushState({title:$(_this.current_element).parent().children('.vdata').data('info')}, '', current_url);
