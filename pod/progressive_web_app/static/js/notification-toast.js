...
async function setPushPreference(notificationSettingUrl) {
    ...
    const notificationButton = document.querySelector("#notification-action-button");
    ...
    let afterBrowserNotificationPermissionChanged = async function (subscription) {
        acceptsNotifications = subscription != null;
        let response = await postNotificationPreference(acceptsNotifications, notificationSettingUrl);

        await updateToast();

        if (!response.ok) {
            notificationsPreferenceTips.textContent = gettext("An error happened during notification subscription");
    ...
    if (permissionState == "prompt") {
        ...
        notificationsPreferenceTips.textContent = gettext("Don't forget to allow notifications from this website in your browser's settings!");
...
async function updateToast() {
    ...
    if (permissionState == "denied") {
        ...
        notificationsPreferenceTips.classList.remove("alert-warning")
        notificationsPreferenceTips.classList.add("alert-error")
        notificationsPreferenceTips.textContent = gettext("You have denied notifications in your browser, to enable them back you must do it in your browser configuration menu.");
        notificationButton.textContent = gettext("Enable notifications")
        notificationButton.disabled = true
    }

    else if (permissionState == "prompt" || (permissionState == "granted" && !subscription)) {
        notificationsPreferenceTips.classList.remove("d-none")
        notificationsPreferenceTips.classList.remove("alert-primary")
        notificationsPreferenceTips.classList.remove("alert-warning")
        notificationsPreferenceTips.classList.add("alert-warning")
        notificationsPreferenceTips.textContent = gettext("Notifications are currently disabled.");
        notificationButton.textContent = gettext("Enable notifications")
        ...
        notificationsPreferenceTips.textContent = gettext("Notifications are currently enabled.");
